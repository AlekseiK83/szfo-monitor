...
# Whitelist ключевых слов, отличающих заголовок звания от названий
# организаций/партий («ЕДИНАЯ РОССИЯ», «ПОБЕДА», «СИБИРЬ» и т.п.).
# Если содержимое кавычек НЕ содержит ни одного из этих слов — считаем,
# что это не заголовок награды.
AWARD_KEYWORDS_QUOTED = re.compile(
    r'ЗАСЛУЖЕНН|НАРОДН|ПОЧ[ЁЕ]ТН|МАТЬ-ГЕРОИНЯ|РОДИТЕЛЬСК'
    r'|ЗА\s+ЗАСЛУГИ|ЗА\s+СПАСЕНИЕ|ЗА\s+ОТВАГУ|ЗА\s+ТРУДЫ'
    r'|ЗА\s+БЕЗУПРЕЧНУЮ|ЗА\s+БЛАГОДЕЯНИЕ|ЗА\s+НАСТАВНИЧЕСТВО'
    r'|ЗА\s+РАЗВИТИЕ|ЗА\s+ВЕРНОСТЬ|ЗА\s+БОЕВЫЕ'
    r'|ПУШКИНА|ЛУКИ\s+КРЫМСКОГО|ПИРОГОВА|АЛЕКСАНДРА\s+НЕВСКОГО|ДРУЖБЫ'
    r'|СЛАВА|ОТЕЧЕСТВОМ'
)

# Splitter общего вида (для указов): «Присвоить …:», «Наградить …:»,
# «За [что-то] наградить/присвоить/объявить [что-то]». Просто сбрасывает
# текущую награду — далее ожидается заголовок звания или ордена.
SPLITTER = re.compile(
    r'^\s*('
    r'Присвоить(?:\s+поч[её]тн[а-я]+)?\s+звани[ея]\s*:?'
    r'|Наградить(?:\s+[^\n]{0,300})?'
    r'|За\s+[^\n]{5,500}(?:наградить|присвоить)(?:\s+[^\n]{0,300})?'
    r')\s*$',
    re.IGNORECASE
)

# Specialized splitters для распоряжений — не только сбрасывают cur_award,
# но и УСТАНАВЛИВАЮТ его, потому что после них нет заголовка звания —
# сразу идут ФИО награждаемых.
GRAMOTA_MARKER = re.compile(
    r'^\s*(?:За\s+[^\n]{5,500}\s+)?наградить\s+Поч[её]тной\s+грамотой'
    r'(?:\s+[^\n]{0,300})?\s*$',
    re.IGNORECASE
)
BLAGODARNOST_MARKER = re.compile(
    r'^\s*(?:За\s+[^\n]{5,500}\s+)?объявить\s+благодарность'
    r'(?:\s+[^\n]{0,300})?\s*$',
    re.IGNORECASE
)
GRAMOTA_AWARD = "Почётная грамота Президента Российской Федерации"
BLAGODARNOST_AWARD = "Благодарность Президента Российской Федерации"
...
SZFO_REGIONS = [
    ...
    ("Республика Коми", "respublika-komi", [
        ...
        r"\bсыктывкар(?:а|у|е|ом|ский|ская|ского|ской|скому|ским|ском)?\b",
        r"\bухт(?:а|ы|у|е|ой|инский|инская|инского|инской|инскому|инским|инском)\b",
        r"\bворкут(?:а|ы|у|е|ой|инский|инская|инского|инской|инскому|инским|инском)\b",
        r"печор(?:а|ы|у|е|ой|ский|ская)\s+(?:коми|город|район|мест)",  # с уточнением
        r"город\s+печор(?:а|ы|у|е|ой)",
        # Инта — очень короткое, требуем границы слова (иначе "интенсивный", "интернет", "интерес")
        r"\bинт(?:а|ы|у|е|ой|инский|инская|инского|инской|инскому|инским|инском)\b",
        r"\bусинск(?:а|у|е|ом|ий|ая|ого|ой|ому|им)?\b",
        r"\bсосногорск(?:а|у|е|ом|ий|ая|ого|ой|ому|им)?\b",
        r"\bвуктыл(?:а|у|е|ом|ский|ская|ского|ской)?\b",
        r"\bемв(?:а|ы|у|е|ой|инский|инская|инского|инской)\b",
        r"\bмикунь(?:я|ю|ем|е|ский|ская|ского|ской)?\b",
    ]),
